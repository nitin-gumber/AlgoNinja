...
export const getJudge0LanguageName = async (languageId) => {
  try {
    ...
    // Find the language id matches the given languageId

    const language = data.find((lang) => lang.id === languageId);

    return language;
  } catch (error) {
    console.error(`Error When getting language name: ${error.message}`);
    return null;
  }
};
